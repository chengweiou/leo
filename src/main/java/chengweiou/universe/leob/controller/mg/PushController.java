...
public class PushController {
    ...
    public Rest<Long> push(Push e) throws ParamException, FailException {
        ...
        List<Device> deviceList = dio.find(new SearchCondition(), Builder.set("person", e.getPerson()).to(new Device()));
        List<String> tokenList = deviceList.stream().filter(device -> device.getActive()).map(Device::getToken).toList();
